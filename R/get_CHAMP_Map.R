...
#'
#' This function is a wrapper for other CHAMP_Map_scripts helper functions to
#' get the iterative map for the resolution parameter on the CHAMP set of
#' partitions and their corresponding domains of optimality.
#'
#' @param network A network object representing the structure of the network.
#' @param partitions A list containing the different partitions.
#' @param partition_summary A data frame containing the summary of the partitions,
#'                          obtained by CHAMP.R.
#' @param plottitle (Optional) A title for the plot.
#'
#' @return A data frame containing the updated partition_summary with iterative
#'         map information for all partitions.
#'
#' @export
#'
#' @author Rachel Matthew, Ryan Rebne, Ava Scharfstein, PJM
#'
#' @examples
...
get_CHAMP_Map <- function(network,
  ...
  for (x in 1:lengths(partition_summary)[1]) {
    if (!is.na(partition_summary[x, "next_gamma"]) &&
      ...
      #                  "clusters", sep = " "))
      print(paste(
        "Partition #", pnum, "(with", partition_summary[x, "num_clusters"],
  ...
  ggfig <- ggfig +
    ...
    geom_line(
      data = plot_data.3, aes(x = x2, y = y2, group = group),
